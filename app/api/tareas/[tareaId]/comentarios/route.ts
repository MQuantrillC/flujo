...
export async function POST(req: Request, { params }: { params: Promise<{ tareaId: string }> }) {
  ...
  if (!t || !esMiembro(t.equipoId, email)) return NextResponse.json({ error: 'No encontrado' }, { status: 404 });

  const fd = await req.formData();
  const texto = String(fd.get('texto') ?? '').trim();
  const archivos = fd.getAll('archivos').filter((a): a is File => a instanceof File && a.size > 0);
  if (!texto && archivos.length === 0) return NextResponse.json({ error: 'Escribe algo o adjunta una imagen.' }, { status: 400 });
  for (const a of archivos) {
    if (!a.type.startsWith('image/')) return NextResponse.json({ error: `«${a.name}» no es una imagen.` }, { status: 400 });
    if (a.size > MAX_BYTES) return NextResponse.json({ error: `«${a.name}» pesa más de 10 MB.` }, { status: 400 });
  }
