...
export function ImagenAdjunta({ adjunto }: { adjunto: Adjunto }) {
  ...
  return (
    <figure className="group relative">
      ...
      <FormConfirmar action={eliminarAdjuntoAccion} mensaje="¿Quitar esta imagen?" className="absolute -right-1.5 -top-1.5 hidden group-hover:block">
        <input type="hidden" name="adjuntoId" value={adjunto.id} />
        <button className="rounded-full bg-gray-800 p-0.5 text-white" title="Quitar"><X size={12} /></button>
      </FormConfirmar>
    </figure>
  );
}
